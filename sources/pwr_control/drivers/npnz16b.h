...
typedef enum {
    ...
    CONTROLLER_STATUS_AGM_DISABLED    = 0b0000000000000000,
    CONTROLLER_STATUS_AGM_ENABLED     = 0b0000100000000000,
...
typedef struct NPNZ16_CONTROLLER{
    ...
    } __attribute__((packed))TriggerControl;
    ...
    } __attribute__((packed))DataProviders;
        
    // Cascaded Function Parameters
    struct {
    ...
    struct {
        ...
        volatile uint16_t SourceNormShift;      // Generic 16-bit wide parameter #4 for advanced control options
    } __attribute__((packed))Advanced;
...
typedef struct NPNZ3216_CONTROLLER{
    // External control and monitoring
    volatile CONTROLLER_STATUS_t Status; // Control Loop Status flags
    ...
    volatile uint16_t GainModulationFactor; // Generic 16-bit wide parameter #1 for advanced control options
    volatile uint16_t GainModulationScaler; // Generic 16-bit wide parameter #2 for advanced control options
    volatile uint16_t AdvParam3; // Generic 16-bit wide parameter #3 for advanced control options
    volatile uint16_t AdvParam4; // Generic 16-bit wide parameter #4 for advanced control options
...
//#pragma message "npnz16b header file has been properly included"

#else
#pragma message "npnz16b header file has been recalled and bypassed"
/* ********************************************************************************/
