...
typedef struct NPNZ_STATUS_s {
    union {
    struct {
        volatile bool flt_clamp_min : 1; // Bit 0: control loop is clamped at minimum output level
        volatile bool flt_clamp_max : 1; // Bit 1: control loop is clamped at maximum output level
